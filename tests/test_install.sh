...
cp "$(cd "$(dirname "$0")/.." && pwd)/dist_files/Launch_RegulationManager.desktop" "$SOURCE_DIR/Launch_RegulationManager.desktop"
cp "$(cd "$(dirname "$0")/.." && pwd)/dist_files/Install_RegulationManager.desktop" "$SOURCE_DIR/Install_RegulationManager.desktop"
printf '#!/bin/bash\n' > "$SOURCE_DIR/start.sh"
...
test -f "$DESKTOP_FILE"
grep -Fq 'Exec=/bin/sh -c "exec \"$(dirname \"$1\")/start.sh\"" dummy %k' "$SOURCE_DIR/Launch_RegulationManager.desktop"
grep -Fq 'Exec=/bin/sh -c "exec \"$(dirname \"$1\")/install.sh\"" dummy %k' "$SOURCE_DIR/Install_RegulationManager.desktop"
