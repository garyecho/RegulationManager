...
### 日志查看
- 菜单：工具 → 查看系统日志（快捷键：Ctrl+L）
- 功能：查看系统运行日志、错误记录、用户操作记录
- 过滤：支持按日志级别、关键词、时间过滤
- 导出：可导出日志文件用于问题排查
- 管理：清理旧日志文件，自动轮转（单个文件最大10MB）
- 详细说明请参考：`docs/logging_guide.md`
...
**Q: 麒麟系统数据“丢失”了？**
程序数据保存在启动目录下的 `data/` 文件夹。如果换了解压目录启动，会使用新目录下的 `data/`，并非数据丢失。请固定从同一目录启动，并定期备份 `data/`。
...
- **移除**: 「安装到开始菜单」与「卸载程序」功能及配套脚本；保留解压即用的快速启动方式（`启动制度管理系统.sh`）

### v1.3.2 (2026-09)

- **新增**: 完善日志记录系统：详细记录用户操作、文档上传/编辑/删除、批量导入等关键操作
- **新增**: 日志查看器：菜单工具 → 查看系统日志（Ctrl+L），支持级别过滤、关键词搜索、日志导出
- **新增**: 日志轮转管理：自动按日期分割日志文件，单个文件最大10MB，保留最近10个备份
- **新增**: 日志统计信息：显示日志文件数量、总大小、今日错误/警告数
- **新增**: 日志清理功能：一键清理30天前的旧日志文件
- **新增**: 增强的错误记录：批量操作详细记录成功/失败/跳过数量，异常信息完整记录
- **新增**: 日志使用指南：`docs/logging_guide.md` 详细说明日志系统使用方法
- **优化**: 代码中添加更详细的日志输出，便于问题定位和性能分析
